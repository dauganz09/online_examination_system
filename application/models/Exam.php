...
class Exam extends CI_Model {
   ...
   $r = $res->row_array();
			...
			$code_array['topic_id'] = $r['topic_id'];
...
function checkShuffle($shuffle){
	...
	function get_exam_by_subject($subj_id){

		 $this->db->select('e.*,es.exam_id');
    			$this->db->from('tbl_exam_subjects es, tbl_exams e');
   					$this->db->where('es.exam_id = e.exam_id');
   					
   					$this->db->where('es.subj_id =',$subj_id);
	...
	function get_percentage($eid){
		$this->db->where('exam_id',$eid);
		$res = $this->db->get('tbl_exams');
		$percent = $res->row_array();
		return $percent['percentage'];
	}

function students_results($examid){
  $result = array();
  $pass = 0;
  $failed = 0;
  $totpoints = $this->Sd->countPoints($examid);
  $percentage = $this->Ex->get_percentage($examid);
  ...
  foreach ($res->result() as $row)
      {
          $stud_ave = ($row->score/$totpoints) * 100;
          if($stud_ave >= $percentage){
              $pass++;
          }else{$failed++;}
      }
